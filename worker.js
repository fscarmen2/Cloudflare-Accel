...
<body class="light-mode">
  ...
  <script>
    ...
    function convertGithubUrl() {
      ...
      githubAcceleratedUrl = 'https://' + currentDomain + '/' + url.hostname + url.pathname;
...
async function handleRequest(request, redirectCount = 0) {
  ...
  // 判断是否为 Docker 镜像请求或 GitHub 请求
  if (!pathParts[0].startsWith('https://')) {
    ...
    // GitHub 请求以 https:// 开头
    targetDomain = new URL(pathParts[0]).hostname;
    targetPath = pathParts.slice(1).join('/') + url.search;
  }
